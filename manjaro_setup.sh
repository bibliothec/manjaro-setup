...
yay -S --noconfirm olive zrythm-git rpcs3 sendanywhere meshroom epsxe visual-studio-code-bin android-studio yuzu-mainline-bin citra-qt-bin scilab-bin youtube-dl-gui-git unreal-engine vita3k-git reaper-bin google-chrome  local-by-flywheel-bin github-desktop aptpac genymotion eclipse-java vmware-workstation webex-bin prospect-mail-bin ttf-times-new-roman ttf-mona chrome-remote-desktop ros-melodic-desktop-full duckstation-git bsnes xemu-git m64p vivado microchip-mplabx-bin microchip-mplabxc8-bin microchip-mplabxc16-bin microchip-mplabxc32-bin gvim-gtk2 wine-stable rstudio-desktop-daily-bin stacer openmodelica qucs logisim-evolution unityhub davinci-resolve teams natron-compositor armorpaint-git quartus-free virtualbox-ext-oracle mujoco-bin storyboarder magicavoxel famistudio makehuman mitsuba2-git pixelorama man-pages-ja

pip install numpy pandas tensorflow sklearn matplotlib pillow openpyxl scrapy selenium Scrapy requests beautifulsoup4 flask pygame opencv-python kivy chainer Keras scipy requests pillow jupyter
